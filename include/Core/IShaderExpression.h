...
#define DECLARE_IF(Base) \
	Base If(const ISEFloat& A, const ISEFloat& B, IString op, const Base& Ok, const Base& Else); \
	Base If(const ISEVec2& A, const ISEVec2& B, IString op, const Base& Ok, const Base& Else); \
	Base If(const ISEVec3& A, const ISEVec3& B, IString op, const Base& Ok, const Base& Else); \
	Base If(const ISEVec4& A, const ISEVec4& B, IString op, const Base& Ok, const Base& Else);
...
{ \
	Ret temp; \
	temp.m_iType=op; \
	temp.m_eExpr.Add(*this); \
	temp.m_eExpr.Add(expr); \
	return temp; \
}
...
Base ShaderExpression::Cos(const Base& val) \
{ \
	Base temp; \
	IShaderExpression::_Cos(val,&temp); \
	return temp; \
} \
Base ShaderExpression::Sin(const Base& val) \
{ \
	Base temp; \
	IShaderExpression::_Sin(val,&temp); \
	return temp; \
} \
ISEFloat ShaderExpression::Dot(const Base& val1, const Base& val2) \
{ \
	ISEFloat temp; \
	IShaderExpression::_Dot(val1,val2,&temp); \
	return temp; \
}\
Base ShaderExpression::Normalize(const Base& val) \
{ \
	Base temp; \
...
Ret ShaderExpression::If(const Base& A, const Base& B, IString op, const Ret& Ok, const Ret& Else) \
{ \
	Ret temp; \
...
#define DEFINE_COMMON_FUNC_BASE_ONE(Base,Name) \
Base ShaderExpression::Name(const Base& A) \
{ \
	Base temp; \
...
Base ShaderExpression::Name(const Base& A, const Base& B) \
{ \
	Base temp; \
...
Base ShaderExpression::Name(const Base& A, const Base& B, const Base& C) \
{ \
	Base temp; \
...
#define DEFINE_COMMON_FUNC(Base) \
	DEFINE_COMMON_FUNC_BASE_ONE(Base,Abs) \
	DEFINE_COMMON_FUNC_BASE_ONE(Base,Floor) \
	DEFINE_COMMON_FUNC_BASE_ONE(Base,Ceil) \
	DEFINE_COMMON_FUNC_BASE_ONE(Base,Fract) \
	...
	DEFINE_COMMON_FUNC_BASE_ONE(Base,Sqrt) \
	DEFINE_COMMON_FUNC_BASE_ONE(Base,InverseSqrt) \
	DEFINE_COMMON_FUNC_BASE_ONE(Base,Exp) \
	DEFINE_COMMON_FUNC_BASE_ONE(Base,Log) \
	DEFINE_COMMON_FUNC_BASE_ONE(Base,Exp2) \
	DEFINE_COMMON_FUNC_BASE_ONE(Base,Log2)
...
namespace Initial
{
	namespace ShaderExpression
	{
		...
		public:
			...
			virtual IString GetGLSLString()/*=0*/;
			VarType GetVarType();
			...
			static void _TextureSample(IString texturename, const ISEVec2& expr, ISEVec4* result);
			static void _TextureCoord(const ISEInt& expr, ISEVec4* result);

			static void _If(IShaderExpression* A, IShaderExpression* B, IString op, IShaderExpression* Ok, IShaderExpression* Else, IShaderExpression* result);
		...
		protected:
			...
			IArray<IShaderExpression>	m_eExpr;

			IString						m_sParameter;
			float						m_fValue[4];
		...
		ISEVec4 TextureSample(IString texturename,ISEVec2 coord=TextureCoord().xy());
