...
IVertex ITriangulator::m_vVert[2];
...
void ITriangulator::Triangulate(IArray<ITriangle>& ret)
{
	...
	for (int i=0;i<m_aTriangle.Count();i++)
		//if (&m_aTriangle[i])
	...
	for (int j=0;j<m_aTriangle.Count();j++)
	{
...
void CALLBACK ITriangulator::combineCallback(GLdouble coords[3], IVertex *vertex_data[4], GLfloat weight[4], IVertex **dataOut )
{
	//printf("combine callback\n");
	IVertex *vertex=new IVertex();
