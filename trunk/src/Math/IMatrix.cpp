...
const float* IMatrix::GetFloatValues()
{
	...
	for (int j=0;j<Height;j++)
			for (int i=0;i<Width;i++)		
				floatdata[i*Height+j]=data[i*Height+j];
...
double& IMatrix::operator()(int col, int line)
{
	if (col>=0 && col<Width && line>=0 && line<Height) 
	{
		return data[col*Height+line];
	}else return _temp;
}
...
Core::IVector3D IMatrix::operator*(const Core::IVector3D& vect)
{
	if (GetWidth()==3 && GetHeight()==3)
	{
		...
		mat(0,2)=vect.z;
		mat = *this*mat;
		return Core::IVector3D(mat(0,0),mat(0,1),mat(0,2));
	}else if (GetWidth()==4 && GetHeight()==4)
	{
		IMatrix mat(1,4);
		mat(0,0)=vect.x;
		mat(0,1)=vect.y;
		...
		mat(0,3)=1.0;
		mat = *this*mat;
		return Core::IVector3D(mat(0,0)/mat(0,3),mat(0,1)/mat(0,3),mat(0,2)/mat(0,3));
	}else{
		ILogger::LogError("Incompatible matrix%dx%d operation with Vector3D\n",GetWidth(),GetHeight());
		return vect;
	}
}
